import type {BaseResponse, InitializeMilestoneRequest, MilestoneUpdateRequest} from "@/model/be/jinear-core";
import {api} from "./api";
...
export const projectMilestoneApi = api.injectEndpoints({
    endpoints: (build) => ({
        ...
        //
        updateMilestoneState: build.mutation<BaseResponse, MilestoneUpdateRequest>({
            query: (body) => ({
                url: "v1/project/milestone/state",
                method: "PUT",
                body: body
            }),
            invalidatesTags: ["v1/project/query/assigned/{workspaceId}", "v1/project/query/all/{workspaceId}", "v1/project/query/archived/{workspaceId}", `v1/project/query/{projectId}`]
...
export const {
    ...
    useUpdateMilestoneStateMutation,
