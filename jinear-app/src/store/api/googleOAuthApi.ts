import type {AuthRedirectInfoResponse} from "@/model/be/jinear-core";
import {api} from "./api";
...
export const googleOAuthApi = api.injectEndpoints({
    endpoints: (build) => ({
        ...
        retrieveAttachMailRedirectInfo: build.query<AuthRedirectInfoResponse, { workspaceId: string }>({
            query: (req: {
                workspaceId: string
            }) => `v1/oauth/google/redirect-info/attach-mail?workspaceId=${req.workspaceId}`,
            providesTags: (_result, _err, req) => [
                {
        ...
        retrieveAttachCalendarRedirectInfo: build.query<AuthRedirectInfoResponse, { workspaceId: string }>({
            query: (req: {
                workspaceId: string
            }) => `v1/oauth/google/redirect-info/attach-calendar?workspaceId=${req.workspaceId}`,
            providesTags: (_result, _err, req) => [
                {
