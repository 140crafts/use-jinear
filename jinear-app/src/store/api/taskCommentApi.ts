import type {BaseResponse, InitializeTaskCommentRequest, PaginatedTaskCommentResponse} from "@/model/be/jinear-core";
import {api} from "./api";
    ...
    endpoints: (build) => ({
        ...
        >({
            query: (req: {
                taskId: string;
                page?: number
            }) => `v1/task/comment/from-task/${req.taskId}?page=${req.page || "0"}`,
...
export const {
    useInitializeTaskCommentMutation,
    useRetrieveTaskCommentsQuery,
