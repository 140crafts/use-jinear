import type {BaseResponse, WorkspaceMemberListingBaseResponse} from "@/model/be/jinear-core";
import {api} from "./api";

export const workplaceMemberApi = api.injectEndpoints({
    endpoints: (build) => ({
        retrieveWorkspaceMembers: build.query<WorkspaceMemberListingBaseResponse, {
            workspaceId: string;
            page?: number
        }>({
