...
import type {BaseResponse, LocaleType, WorkspaceInitializeRequest} from "@/model/be/jinear-core";
...
const NewWorkspaceForm: React.FC<NewWorkspaceFormProps> = ({close, onSuccess}) => {
    ...
    return currentAccount ? (
        <form autoComplete="off" id={"new-workspace-form"} className={styles.form} onSubmit={handleSubmit(submit)}
              action="#">
            ...
            <label className={styles.label} htmlFor={"new-workspace-handle"}>
                ...
                <div className={styles.urlInputsContainer}>
                    <div
                        className={styles.hostInput}>{`${HOST?.replace("https://", "")?.replace("http://", "")}/`}</div>
                    <input
