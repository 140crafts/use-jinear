...
export const workspaceDisplayPreferenceApi = api.injectEndpoints({
  endpoints: (build) => ({
    updatePreferredWorkspace: build.mutation<BaseResponse, { workspaceId: string }>({
    ...
    updatePreferredTeam: build.mutation<BaseResponse, { workspaceId: string; teamId: string }>({
    ...
    updatePreferredWorkspaceWithUsername: build.mutation<BaseResponse, { workspaceUsername: string }>({
