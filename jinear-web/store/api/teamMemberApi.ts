...
export const teamMemberApi = api.injectEndpoints({
  endpoints: (build) => ({
    ...
    //
    joinTeam: build.mutation<BaseResponse, { teamId: string }>({
      query: ({ teamId }) => ({
        url: `v1/team/member/join/${teamId}`,
        method: "POST"
      }),
      invalidatesTags: ["v1/team/member/list/{teamId}", "v1/workspace/activity/filter", "v1/team/member/memberships/{workspaceId}"]
    }),
    //
    leaveTeam: build.mutation<BaseResponse, { teamId: string }>({
      query: ({ teamId }) => ({
        url: `v1/team/member/leave/${teamId}`,
        method: "DELETE"
      }),
      invalidatesTags: ["v1/team/member/list/{teamId}", "v1/workspace/activity/filter", "v1/team/member/memberships/{workspaceId}"]
    }),
...
export const {
  endpoints: { retrieveTeamMembers, kickMemberFromTeam, joinTeam, leaveTeam, addTeamMember, retrieveMemberships }
} = teamMemberApi;
