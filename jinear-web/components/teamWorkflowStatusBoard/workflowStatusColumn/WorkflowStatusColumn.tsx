...
const filterByGroup = (
  ...
  const isThisTaskStatusUpdatePendingTask =
    workflowStatusUpdatePendingTask?.taskId == taskDto.taskId;
  const taskIsInThisGroup =
  ...
  if (isThisTaskStatusUpdatePendingTask) {
    ...
    return (
      workflowStatusDto.teamWorkflowStatusId ==
      workflowStatusUpdatePendingTask.newWorkflowStatusId
    );
  }
...
const WorkflowStatusColumn: React.FC<WorkflowStatusColumnProps> = ({
  ...
  const filteredTasks =
    tasks?.filter((taskDto) =>
      filterByGroup(taskDto, workflowStatusDto, workflowStatusUpdatePendingTask)
    ) || [];
  ...
  return (
    <div className={styles.container}>
      ...
      <Droppable
        key={workflowStatusDto.teamWorkflowStatusId}
        droppableId={workflowStatusDto.teamWorkflowStatusId}
      >
        {(
