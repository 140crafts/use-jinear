...
import { changeLoadingModalVisibility, closeTeamOptionsModal } from "@/store/slice/modalSlice";
...
const TeamList: React.FC<TeamListProps> = ({ preferredWorkspace, preferredTeamId }) => {
  ...
  const _changePreferredTeam = (workspaceId: string, teamId: string, teamName: string) => {
  ...
  return (
    <div className={styles.container}>
      <div className={styles.title}>{t("teamOptionsSelectActiveTeamTitle")}</div>
      <div className={styles.listContainer}>
        {isLoading && <CircularProgress size={15} className={styles.loading} />}
        {isSuccess && teamsResponse.data.length == 0 && <div className={styles.noTeamLabel}>{t("sideMenuTeamListNoTeam")}</div>}
        {isSuccess &&
          teamsResponse.data.map((team) => (
            <Button
              key={`change-team-${team.teamId}`}
              className={cn(styles.button, team.teamId == selectedTeam?.teamId ? styles.currentTeamButton : null)}
              variant={team.teamId != selectedTeam?.teamId ? ButtonVariants.default : ButtonVariants.filled}
              onClick={() => {
