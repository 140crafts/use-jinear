...
const NewReminderModal: React.FC<NewReminderModalProps> = ({}) => {
  ...
  const { register, reset, watch, setValue, handleSubmit } =
    useForm<TaskReminderInitializeForm>();
        ...
        {!hasAnyAssignedDateReminder && taskHasAssignedDate && (
          <>
            <label className={styles.labelRow} htmlFor={"before-assigned-date"}>
              <input
                id={"before-assigned-date"}
                type={"checkbox"}
                {...register("beforeAssignedDate")}
              />
              <b>{t("taskNewReminderModalOnAssignedDateLabel")}</b>
              <div className={styles.formattedDateLabel}>
        ...
        {specificDate && (
          <div className={styles.specificDateContainer}>
            ...
            <label className={styles.label} htmlFor={"repeat-type"}>
              ...
              <select
                id="repeat-type"
                {...register("specificRemindDateRepeatType")}
              >
            ...
            {specificRemindDateRepeatType &&
              specificRemindDateRepeatType != "NONE" && (
                <div className={styles.repeatContainer}>
                  <label
                    className={styles.labelRow}
                    htmlFor={"end-repeat-checkbox"}
                  >
                    <div className="flex-1" />
                    <input
                      id={"end-repeat-checkbox"}
                      type={"checkbox"}
                      {...register("repeat")}
                    />
                  ...
                  {repeat && (
                    <label className={styles.label} htmlFor={"remind-end-date"}>
                      ...
                      <input
                        id={"remind-end-date"}
                        type={"date"}
                        {...register("specificRemindRepeatEnd")}
                      />
                    </label>
                  )}
                </div>
