...
const SecondLevelSideMenu: React.FC<SecondLevelSideMenuProps> = ({ id, className, children, open = true, toggle }) => {
  ...
  return (
    <div id={id} className={cn(styles.sideMenu, open && styles.sideMenuVisible, className)}>
      <div id="second-level-side-menu-action-bar" className={styles.sideMenuActionBar}>
        <Button className={styles.menuToggleButton} heightVariant={ButtonHeight.short} onClick={toggle}
                variant={menuVariant}>
      ...
      <div id="second-level-side-menu-content"
           className={cn(styles.sideMenuContent, open && styles.sideMenuContentVisible)}>
        {children}
      </div>
    </div>
  );
