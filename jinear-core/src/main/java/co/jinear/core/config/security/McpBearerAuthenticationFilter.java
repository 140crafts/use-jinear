...
/**
 * Establishes the caller identity for MCP requests.
 * <p>
...
public class McpBearerAuthenticationFilter extends OncePerRequestFilter {
    ...
    private final McpAccessTokenResolver mcpAccessTokenResolver;
    ...
    protected void doFilterInternal(@NonNull HttpServletRequest request,
        ...
        if (Objects.isNull(SecurityContextHolder.getContext().getAuthentication())) {
            mcpAccessTokenResolver.extractBearer(request.getHeader(AUTHORIZATION))
                    .flatMap(mcpAccessTokenResolver::resolve)
     ...
     */
    private void setAuthentication(McpAccessTokenVo vo) {
    ...
    public static Optional<McpAccessTokenVo> currentAccessToken() {
        return Optional.ofNullable(SecurityContextHolder.getContext().getAuthentication())
                .map(authentication -> authentication.getDetails())
                .filter(McpAccessTokenVo.class::isInstance)
                .map(McpAccessTokenVo.class::cast);
    }
}
