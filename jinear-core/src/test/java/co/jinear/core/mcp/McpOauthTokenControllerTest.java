...
import co.jinear.core.controller.mcp.McpOauthTokenController;
...
class McpOauthTokenControllerTest {
    ...
    @BeforeEach
    void setUp() {
        tokenManager = Mockito.mock(McpTokenManager.class);
        McpOauthTokenController controller = new McpOauthTokenController(tokenManager, new McpOauthErrorMapper());
    ...
    void reportsADeadGrantAsInvalidGrant() throws Exception {
        ...
        Mockito.when(tokenManager.token(Mockito.anyMap()))
                .thenThrow(new BusinessException("mcp.error.oauth.invalid-grant"));
    ...
    void reportsAnUnknownClientAsInvalidClientWithA401() throws Exception {
        Mockito.when(tokenManager.token(Mockito.anyMap()))
                .thenThrow(new BusinessException("mcp.error.oauth.invalid-client"));
