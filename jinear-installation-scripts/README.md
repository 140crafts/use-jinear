...
| Variable | Purpose |
|----------|---------|
| `MCP_ENABLED` | `true` runs the MCP server, `false` turns it off |
| `MCP_JWT_SECRET` | Signs MCP access tokens. Generated by the installer |
| `MCP_DCR_ENABLED` | `true` lets a client register itself. Leave on unless you pin clients |
| `MCP_CIMD_ALLOWED_HOSTS` | Comma separated hosts allowed to describe a client. Empty accepts any public https host |
...
- **`MCP_JWT_SECRET` is not `JWT_SECRET`.** They are separate on purpose: an assistant's
  token must never be able to open a browser session, and a session cookie must never be
  able to call a tool.
