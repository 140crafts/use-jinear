...
| Variable | Purpose |
|----------|---------|
| `MCP_ENABLED` | `true` runs the MCP server. `false` makes the tool endpoint answer 404 |
| `OAUTH_ENABLED` | `true` runs the OAuth authorization server. `false` stops authorization and token refresh |
...
- **There are two switches, and a member needs both.** `OAUTH_ENABLED` runs the
  authorization server that issues tokens; `MCP_ENABLED` runs the tool endpoint those
  tokens open. The installer sets both from one question. Turning off only `MCP_ENABLED`
  leaves an authorization server with nothing to authorize; turning off only
  `OAUTH_ENABLED` leaves a tool endpoint nobody can authenticate to.
